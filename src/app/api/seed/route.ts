...
// Specific images for each product type
...
function generateProducts(count: number) {
    ...
    for (let i = 0; i < count; i++) {
        ...
        const typeList = productTypes[category as keyof typeof productTypes];
        const type = typeList[Math.floor(Math.random() * typeList.length)];
        const adjective = adjectives[Math.floor(Math.random() * adjectives.length)];

        // Get specific image for this product type
...
export async function GET() {
    try {
        ...
        // Clear existing products to avoid duplicates if re-seeding or just append?
        // User asked to "add" products, but usually seed resets or adds if empty.
        // Let's check count. If low, add more. If high, maybe skip?
        // To ensure we get 30-40, let's just add them.

        const products = generateProducts(40);
        await Product.insertMany(products);

        return NextResponse.json({ message: 'Database seeded successfully with 40 new products', count: products.length });
    } catch (error) {
